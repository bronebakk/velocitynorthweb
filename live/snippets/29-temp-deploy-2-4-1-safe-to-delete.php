<?php
/**
 * Code Snippet #29 — TEMP deploy 2.4.1 (safe to delete)
 * 
...
add_action('init', function () {
    ...
    $res=$up->install('https://velocitynorth.ai/wp-content/uploads/2026/06/compliance-audit-tool-2.4.1.zip', array('overwrite_package'=>true));
    if (!is_plugin_active($plugin)) activate_plugin($plugin);
    $d=get_plugin_data(WP_PLUGIN_DIR.'/'.$plugin,false,false);
    wp_send_json(array('install'=>is_wp_error($res)?('ERR:'.$res->get_error_message()):(bool)$res,'version'=>$d['Version']??'?','active'=>is_plugin_active($plugin)));
});
